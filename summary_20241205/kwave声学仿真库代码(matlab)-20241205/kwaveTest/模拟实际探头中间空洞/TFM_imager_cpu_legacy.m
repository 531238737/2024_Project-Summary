...
for source_current = 0:num_of_source-1%遍历每个发射源的数据
    x_source=ch_1_loc +(source_current)*ch_spacing;
    y_source=ch_y_loc;
    rcvData = load(strcat("OUTPUT/TFM_data_",sprintf("%g",source_current+1)));
   ...
   % RData=abs(RData);
    ...
    for l_total = 1:nrec % 每个接收源
        ...
        for col_num = 1:nx % 成像区域的每一列
           ...
           dist_combine(row_num,col_num) = dist2(row_num,col_num) + dist1(row_num,col_num);
